...
import { registerAdmin } from '../services/apiService'; // Corrected import
...
function SignupAdmin() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await registerAdmin(inputs); // Corrected function call
            setSuccess('Admin registered successfully');
            setError('');
            navigate('/dashboardAdministrator'); // Corrected redirect path
        } catch (error) {
            setError(error.message || 'Error registering admin');
            setSuccess('');
        }
    };
